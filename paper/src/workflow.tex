...
\begin{figure*}[t]
  ...
  \Description{Horizontal workflow from AFAR drop ingestion through module
...
\begin{table*}[t]
  ...
  \caption{Workflow stages, artifacts, and readiness gates.}
  \label{tab:workflow-stages}
  \begin{tabular}{p{0.18\textwidth} p{0.33\textwidth} p{0.40\textwidth}}
    \hline
    Stage & Inputs & Outputs/Gate \\
    \hline
    Drop ingestion & AFAR drop install, version mapping & Updated versions file,
    regenerated module tree. \\
    Module generation & Config files, MPICH paths & Modulefiles and
    `pkg-config` metadata for AFAR and MPI. \\
    Repro tests & Module stack, repro cases & P/S/F outcomes and logs; drop is
    blocked on FAIL. \\
    Validation suites & OLCF Fortran apps, SOLLVE V\&V & Broader functional
    checks; results recorded for rollout. \\
    User-ready release & PASS/SKIP gates and suite summaries & Module tree
    staged for early adopters with readiness report. \\
    \hline
  \end{tabular}
\end{table*}
...
These logs are paired with the generator output (modulefiles and `pkg-config`
metadata) so each AFAR drop can be traced to the exact environment used for
validation. This pairing allows support staff to hand off a drop with
reproducible evidence of readiness rather than informal notes.

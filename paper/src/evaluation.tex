...
\begin{table}[t]
  ...
  recorded as pass/skip/fail (P/S/F) for each test.}
  \label{tab:repro}
  \begin{tabular}{l l l}
    \hline
    Test & Coverage & Status (P/S/F) \\
    \hline
    cmake-mpi-omp & CMake MPI/OpenMP & PASS \\
    ...
    omp-f90 & OpenMP offload (Fortran) & PASS \\
    libsci & PE math libs & PASS \\
    libsci-acc & accelerator libs & SKIP \\
    hdf5-parallel & HDF5 (parallel) & SKIP \\
    hdf5-serial & HDF5 (serial) & PASS \\
...
The `libsci-acc` profile is intentionally skipped because \texttt{libsci\_acc} linking
is not yet validated and may depend on ROCm 6 runtime libraries. The parallel
...
and library metadata regardless of module ordering. We report pass/skip/fail
outcomes per permutation and flag any regressions for follow-up.
...
\begin{table}[t]
  ...
  \begin{tabular}{l l l}
    ...
    \hline
    Libraries before AFAR & load libs, then afar-prgenv & PASS \\
    Libraries after AFAR & load afar-prgenv, then libs & PASS \\
...
OpenMP offload tests confirm that `--offload-arch` is injected consistently for
both C and Fortran workflows when `craype-accel-amd-gfx90a` is loaded (or when
\texttt{AFAR\_\*\_OFFLOAD\_ARCH} overrides are set).
The `mpi-hello`, `omp-c`, and `omp-f90` repros all pass in this evaluation.

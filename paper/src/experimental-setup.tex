...
`pkg-config`, and \texttt{scripts/check\_ldd.sh} validates the resulting binaries.
...
diagnostic completeness of wrapper logs, pass/skip/fail rates per test, and the
time to validate a new drop. We also track module-order robustness by running
the repro suite under multiple module load permutations to ensure metadata
